#! python3
# -*- coding: utf-8 -*-
"""Re-link broken ACC cloud links by NAME, keeping them on the cloud.

Why this exists
---------------
In the "Consumed" coordination workflow every publish creates a NEW item in
ACC, i.e. a new Model GUID. A Revit link stores the old GUID, so it breaks
(``LinkNotFound``) and ``Reload()`` cannot fix it. Re-linking by local path
would convert the link to a LOCAL file link (bad for the team).

This tool looks up the CURRENT Model GUID of each file by name straight from
the LOCAL Desktop Connector cache (no APS API, no Account Admin approval - so
it works even when the ACC belongs to a client), rebuilds the cloud ModelPath
and re-links it ON THE CLOUD via ``RevitLinkType.LoadFrom``.

Requirements
------------
* Autodesk Desktop Connector installed and signed in.
* The project synced in Desktop Connector at least once (so the local index
  knows the current Model GUIDs).

NOTE: this reads Desktop Connector's internal/undocumented store via
``lib/dc_local.py``. It works today (Desktop Connector 16.x) but Autodesk may
change it without notice.
"""

__title__ = "ReLink\nCloud"
__doc__ = (
    "Re-link broken ACC cloud links by name using the local Desktop Connector "
    "cache. Keeps links on the cloud (no local conversion). Preview first."
)
...
# Active document (guarded: ActiveUIDocument is None when no project is open).
_uidoc = __revit__.ActiveUIDocument  # noqa: F821
doc = _uidoc.Document if _uidoc is not None else None


# ============================================================
# UI / CONFIG HELPERS (no pyrevit dependency - runs on CPython)
# ============================================================
...
def _log(text):
    """Best-effort append to a rolling log for post-mortem troubleshooting."""
    try:
        if not os.path.isdir(_CFG_DIR):
            os.makedirs(_CFG_DIR)
...
def _select_many(labels, preselect, title="Selecione"):
    """labels: list[str]; preselect: list[bool]. Returns list of indices."""
    f = Form()
    f.Text = title
    f.StartPosition = FormStartPosition.CenterScreen
...
def _pick_folder(prompt, initial=None):
    """Folder browser. Returns the chosen path string or None."""
...
def _link_type_name(link_type):
    try:
        ...
        try:
            ...
            name = ""
    # Strip a possible " : position N" suffix some links carry.
    if " : " in name:
        name = name.split(" : ")[0]
    return name.strip()
...
def _name_candidates(name):
    """Return possible index keys for a link name (with/without .rvt)."""
    low = name.lower()
    keys = [low]
...
def _make_cloud_path(region, project_guid_str, guid_bytes):
    ...
    # Prefer the region-aware signature (Revit 2021+); fall back to the older
    # US-only one. Only try the region signature when a region is known.
    if region:
        try:
...
def main():
    ...
    if doc.IsFamilyDocument:
        _alert("Este comando roda em um PROJETO, nao em uma familia.",
               exit_after=True)
    ...
    # ========================================================
    # 1. BUILD THE LOCAL INDEX FROM DESKTOP CONNECTOR
    # ========================================================
    try:
        index = dc_local.build_index()
    except Exception as ex:
        _alert("Nao consegui ler o cache do Desktop Connector:\n\n%s\n\n"
               "Verifique se o Desktop Connector esta instalado, logado e com "
    ...
    # ========================================================
    # 2. IDENTIFY HOST PROJECT + REGION (and validate the GUID decode)
    # ========================================================
    project_id = None
    region = None
    validation_msg = "Validacao do host: nao foi possivel comparar."
    ...
    if host_visible:
        ...
        for cand in dc_local.lookup(index, host_name):
            ...
            if built_visible == host_visible:
                ...
                region = cand["region"]
                validation_msg = "Validacao do host: OK (GUID bate)."
                break
        if project_id is None:
            validation_msg = ("Validacao do host: nao encontrei o host no "
                              "cache (nao foi possivel confirmar o projeto "
                              "automaticamente).")

    # Fallback: let the user pick the project from the local index.
    if project_id is None:
        projects = dc_local.distinct_projects(index)
        # Build a readable label with one sample file name per project.
        sample = {}
    ...
    # ========================================================
    # 2b. OPTIONAL: point to a specific cloud folder (e.g. Consumed)
    # ========================================================
    # The cloud path of a link is only region + project + model GUID; the
    # FOLDER is not part of it. So "indicate the folder" means: when the same
    # file name exists in several folders (e.g. Shared vs Consumed), take the
    # GUID from the folder the user points to. The user picks the LOCAL Desktop
    # Connector folder; we map it to the cloud folder by file-name overlap.
    folder_urn = None
    ...
    if _confirm("Quer indicar a pasta de onde puxar os links "
                ...
                "Nao = usar o item mais recente do projeto",
        ...
        if chosen:
            ...
            if not info:
                _alert("Nao consegui casar essa pasta com o cache do ACC.\n"
                       "Verifique se ela contem arquivos .rvt sincronizados "
    ...
    # ========================================================
    # 3. RESOLVE EACH LINK -> CLOUD PATH (preview)
    # ========================================================
    link_types = list(FilteredElementCollector(doc).OfClass(RevitLinkType))
    if not link_types:
        _alert("Nenhum link Revit neste projeto.", exit_after=True)
    ...
    for lt in link_types:
        ...
        if not matches:
            if folder_urn:
                note = "NAO encontrado na pasta indicada"
            else:
                note = "NAO encontrado no cache do projeto"
        else:
            # Entries are newest-first: pick the current (latest) item.
            entry = matches[0]
            # Use the region / project GUID stored on the entry itself; fall
            # back to the host's values. This is correct even if a link lives
            # in a different region/hub than the host.
            ...
            try:
                ...
                note = "ERRO resolvendo GUID: %s" % ex
    ...
    _alert("{0}\n{1}\n\nLinks prontos para re-linkar: {2} de {3}.\n\n"
           "Na proxima janela, marque os que deseja aplicar.".format(
               validation_msg, folder_label, len(ready_rows), len(rows)),
           title="ReLink Cloud - preview")
    ...
    if not to_apply:
        _alert("Nenhum dos selecionados esta pronto para re-linkar.",
               exit_after=True)

    if not _confirm(
            "Vai re-linkar %d link(s) na NUVEM.\n\n"
            "Isso altera o modelo aberto (mas nao sincroniza).\n"
            "Recomendado testar em 1 modelo antes dos 60.\n\nContinuar?"
            % len(to_apply)):
        raise _ScriptExit()

    # ========================================================
    # 8. APPLY (LoadFrom must run OUTSIDE a transaction)
    # ========================================================
    wc = _workset_config()
    ...
    for r in to_apply:
        ...
        try:
            path = r["cloud_path"]
            if path is None:
                raise ValueError("cloud path ausente")
            # LoadFrom(path, wc) is preferred; fall back to the overload
            # without a WorksetConfiguration if we could not build one.
            if wc is not None:
        ...
        _log("  [%s] %s -> %s" % ("OK" if ok else "ERR", r["name"],
                                  result_text))
    ...
    _log("Concluido: %d/%d OK." % (ok_count, len(results)))

    td = TaskDialog("ReLink Cloud - relatorio")
...
try:
    ...
    _log("ERRO NAO TRATADO:\n" + tb)
    try:
        td = TaskDialog("ReLink Cloud - erro")
        td.MainInstruction = "Ocorreu um erro inesperado."
        td.MainContent = ("O processo foi interrompido. Links ja aplicados "
