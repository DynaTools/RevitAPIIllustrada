# -*- coding: utf-8 -*-
"""Auxiliar do Autodesk Platform Services (APS / Forge) para o Data Management do ACC.

Implementação só com a biblioteca padrão (sem pacotes externos), para rodar no
engine CPython do pyRevit. Oferece:

  * OAuth 3-legged (PKCE, cliente público - sem client secret),
  * um pequeno servidor HTTP local para capturar o redirecionamento do OAuth,
  * cache / renovação do token em %APPDATA%,
  * navegação no Data Management (hubs, projetos, pastas, itens),
  * auxiliares para transformar a URN de linhagem de um item do ACC nos bytes
    de GUID de que o Revit precisa para montar um ModelPath de nuvem.

O ModelPath de nuvem em si precisa ser montado do lado do Revit (ele exige a
API do Revit), usando `lineage_urn_to_guid_bytes()` + o GUID do projeto.
"""
...
try:
    # Python 3 (engine CPython do pyRevit)
    import urllib.request as _urlreq
    import urllib.parse as _urlparse
    from urllib.error import HTTPError, URLError
    from http.server import BaseHTTPRequestHandler, HTTPServer
except ImportError:  # pragma: no cover - alternativa IronPython
    raise
...
class _RedirectHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):  # noqa: N802 (nomenclatura da stdlib)
        ...
        msg = ("<html><body style='font-family:Segoe UI;text-align:center;"
               "margin-top:60px'><h2>Autenticação concluída.</h2>"
...
def _run_redirect_server(host, port, timeout):
    ...
    def _serve():
        # Atende uma única requisição (o redirecionamento) e para.
...
# ----------------------------------------------------------------------------
# OAuth 3-legged (PKCE, cliente público)
# ----------------------------------------------------------------------------
...
def login_interactive(client_id, redirect_uri, scopes=DEFAULT_SCOPES,
                      timeout=180):
    """Executa o fluxo PKCE 3-legged completo e devolve o access token (string)."""
    ...
    if captured.get("error"):
        raise RuntimeError("Erro de autorização: %s - %s"
                           % (captured.get("error"),
                              captured.get("error_description", "")))
    if captured.get("state") != state:
        raise RuntimeError("State inválido (possível CSRF). Tente novamente.")

    code = captured.get("code")
    if not code:
        raise RuntimeError("Nenhum 'code' devolvido pela Autodesk.")
...
def get_access_token(client_id, redirect_uri, scopes=DEFAULT_SCOPES):
    """Devolve um access token válido, renovando-o ou fazendo login quando preciso."""
    ...
    if cache.get("refresh_token"):
        try:
            ...
            pass  # segue para o login interativo

    return login_interactive(client_id, redirect_uri, scopes)


# ----------------------------------------------------------------------------
# Navegação no Data Management
# ----------------------------------------------------------------------------
def _get_all(url, token):
    """GET seguindo a paginação JSON:API; devolve (data[], included[])."""
    data = []
    included = []
    while url:
...
def list_folder_contents(token, project_id, folder_id):
    """Devolve (subpastas, itens). Os itens trazem o nome de exibição da última versão."""
...
def build_name_index(token, project_id, root_folder_id, progress=None,
                     max_folders=5000):
    """Indexa recursivamente todos os itens sob uma pasta.

    Devolve {lower_name: [item_id, ...]}, para que nomes duplicados possam ser
    detectados. `progress` é um callable opcional(count, current_folder_name).
    """
    index = {}
...
# ----------------------------------------------------------------------------
# URN -> bytes do GUID (para o ConvertCloudGUIDsToCloudPath do Revit)
# ----------------------------------------------------------------------------
def lineage_urn_to_guid_bytes(item_id):
    ...
    Exemplo de item_id: 'urn:adsk.wipprod:dm.lineage:AbCdEf...'
    O segmento final é o base64 URL-safe do GUID de 16 bytes do modelo, na
    ordem de bytes do System.Guid do .NET, então pode ir direto para System.Guid(bytes).
    """
    seg = item_id.split(":")[-1]
    # base64url, restaura o padding
    seg += "=" * (-len(seg) % 4)
...
def project_id_to_guid_str(project_id):
    """Id de projeto do ACC 'b.<guid>' -> '<guid>' para o ConvertCloudGUIDs do Revit."""
